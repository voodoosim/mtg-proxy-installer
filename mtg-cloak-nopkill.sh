...
echo "🚀 MTG + Cloak 3명용 프록시 설치 (No Pkill)"
echo "============================================"

cd /root

# 기존 프로세스 정리 (pkill 대신 다른 방법)
echo "[LOG] 기존 프로세스 정리 중..."

# ps와 kill을 사용한 프로세스 정리
MTG_PIDS=$(ps aux | grep '[m]tg run' | awk '{print $2}')
if [ -n "$MTG_PIDS" ]; then
    echo "$MTG_PIDS" | xargs kill -9 2>/dev/null || true
    echo "[LOG] MTG 프로세스 정리됨"
fi

CLOAK_PIDS=$(ps aux | grep '[c]k-client' | awk '{print $2}')
if [ -n "$CLOAK_PIDS" ]; then
    echo "$CLOAK_PIDS" | xargs kill -9 2>/dev/null || true
    echo "[LOG] Cloak Client 프로세스 정리됨"
fi

# 파일 정리
rm -f mtg-user*.toml mtg-user*.log cloak-client.log 2>/dev/null || true
...
# ps와 kill을 사용한 프로세스 정리
MTG_PIDS=$(ps aux | grep '[m]tg run' | awk '{print $2}')
[ -n "$MTG_PIDS" ] && echo "$MTG_PIDS" | xargs kill -9 2>/dev/null || true

CLOAK_PIDS=$(ps aux | grep '[c]k-client' | awk '{print $2}')
[ -n "$CLOAK_PIDS" ] && echo "$CLOAK_PIDS" | xargs kill -9 2>/dev/null || true
